...
namespace SimpleNeuralNetwork
{
    class NeuralNetwork
    {
        ...
        {
            var results = new double[_outputLayer.Neurons.Length - 1];
            for (int m = 0; m < inputs.Length; m++)
            {
                for (int j = 0; j < _inputLayer.Neurons.Length - 1; j++)
                {
                ...
                {
                    _outputLayer.Neurons[j].CalculateOutput();
                    results[j] = _outputLayer.Neurons[j].Output;
                }
            }
            return results;
        }
            ...
            for (int i = _hiddenLayers.Length - 1; i >= 0; i--)
            {
                ...
                for (int j = 0; j < neurons.Length - 1; j++)
                {
                    ...
                    for (int k = 0; k < connections.Length; k++)
                    {
                        ...
                        for (int l = 0; l < _outputLayer.Neurons.Length - 1; l++)
                        {
                            var prevNeuron = _outputLayer.Neurons[l];
                            var wjk = prevNeuron.Connections[j].Weight;

                            var output = expectedOutput[i];

                            sum += -neuron.Output * (1 - neuron.Output) * //sigmoid derivative
                                                    connection.Neuron.Output *
                                                    (expectedOutput[l] - neuron.Output) * wjk; //output error
